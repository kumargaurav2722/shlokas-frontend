...
import { getChalisas } from "../../services/devotion.service";

const CHALISAS = [
  {
    id: "hanuman",
    title: "Hanuman Chalisa",
    description: "Strength, devotion, and protection.",
    deity: "Hanuman",
    type: "Chalisa",
    tags: ["hanuman", "strength", "devotion"],
    textUrl: "",
    pdfUrl: ""
  },
  {
    id: "durga",
    title: "Durga Chalisa",
    description: "Shakti, courage, and victory.",
    deity: "Durga",
    type: "Chalisa",
    tags: ["durga", "shakti", "courage"],
    textUrl: "",
    pdfUrl: ""
  },
  {
    id: "shiv",
    title: "Shiv Chalisa",
    description: "Calmness, balance, and wisdom.",
    deity: "Shiva",
    type: "Chalisa",
    tags: ["shiv", "peace", "wisdom"],
    textUrl: "",
    pdfUrl: ""
  }
];
...
export default function Chalisas() {
  const [query, setQuery] = useState("");
  const [selectedDeity, setSelectedDeity] = useState("");
  const [selectedType, setSelectedType] = useState("");
  const [items, setItems] = useState(CHALISAS);
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        if (active) {
          ...
          setItems(data.length ? data : fallback ? CHALISAS : []);
        }
      } catch (err) {
        if (active) {
        ...
        <div className="mx-auto max-w-6xl">
          <div className="text-center">
            ...
            <p className="mt-3 text-muted">
              Text and PDF formats will appear once backend integrations are
              connected.
            </p>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-muted">
                Search by deity, mood, or intention.
              </p>
              {error && (
          ...
          {!loading && filtered.length === 0 && (
            <p className="mt-8 text-sm text-muted">
              No matching chalisas found. Try a different keyword.
            </p>
          )}
        </div>
      </section>
