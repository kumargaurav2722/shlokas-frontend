...
export default function Verse({ verse }) {
  ...
  const { explain, loading, error, data } = useExplain();
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            onClick={async () => {
              ...
              const result = await explain(texts, activeLang);
              setExplainData(result);
              setShowExplain(true);
            }}
            className="px-3 py-1 rounded-full text-xs font-semibold border border-amber-200 text-amber-900 bg-white/70"
          >
            {getTranslation('common.explain_more') || 'Explain More'}
          </button>
          <AudioPlayer
            textId={verse.id}
            language={activeLang}
